...
import { Face, Mesh3D, Vertex3D } from "./component/mesh.js";
...
const zero_dimension = (p) => {
    const canvas = document.querySelector("#zero-dimension");
    p.setup = function () {
        p.createCanvas(200, 50, p.P2D, canvas);
...
const two_dimension = (p) => {
    ...
    p.setup = function () {
        p.createCanvas(200, 200, p.P2D, canvas);
        p.pixelDensity(p.displayDensity());
        p1.x = p2.x = 100;
        yslider.elt.min = `${0}`;
        yslider.elt.max = `${100}`;
...
const four_dimension = (p) => {
    ...
    function updateAxisMesh(axisModel) {
        axisScreenPositions = projectToScreen(axisModel, p, camera3D, camera4D);
    }
    function displayTessract(tesseract) {
        // update w
...
const cube_ortho = (p) => {
    ...
    p.setup = function () {
        ...
        //init cube mesh
        {
            ...
            cubeShadow.mesh.vertice = new Array();
    ...
    function orthoProjectToPlane(cube, cubeShadow) {
...
const three_dimension_perspect_cube_face = (p) => {
    ...
    function perspectProjectToPlane(p, camera, cube, cubeShadow) {
        cubeShadow.mesh.vertice.splice(0);
...
const three_dimension_perspect_cube_rotate_a_round = (p) => {
    ...
    function updateAngle(cubeRotation) {
        const yaw = angleSlider.value() * Math.PI / 180.0;
...
const cube_intersection = (p) => {
    ...
    function updateZ(transform) {
        transform.translate[2] = zSlider.value();
    }
    p.mouseDragged = (e) => {
        if (e.target != canvas)
            return;
