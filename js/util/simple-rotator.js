...
function SimpleRotator(canvas, callback, viewDistance, rotY, rotX) {
    ...
    function SetEventListener(canvas) {
        canvas.addEventListener("mousedown", doMouseDown, false);
        canvas.addEventListener("touchstart", doTouchStart, false);
        // 取消鼠标滚动页面
        if (canvas.addEventListener) {
            canvas.addEventListener('mousewheel', doScroll, false); // 标准事件
            canvas.addEventListener('DOMMouseScroll', doScroll, false); // Firefox
        } else {
            canvas.attachEvent('onmousewheel', doScroll);
        }
    }
