...
import { Edge, Mesh3D, Mesh4D, Vertex3D, Vertex4D } from "../component/mesh.js";
...
const vertexSize = 6;
const axisColor = [128];
function drawAxis(p, v, v0, axisName, color, distance = 20) {
    const d = vec2.create();
    vec2.sub(d, v, v0);
    vec2.normalize(d, d);
...
function displayAxis(p, vertice) {
    const v0 = vertice[0];
    const vx = vertice[1];
    const vy = vertice[2];
    const vz = vertice[3];
    ...
    p.pop();
    if (vx != undefined)
        drawAxis(p, vx, v0, 'X', xColor);
    if (vy != undefined)
        drawAxis(p, vy, v0, 'Y', yColor);
    if (vz != undefined)
        drawAxis(p, vz, v0, 'Z', zColor);
    if (vw != undefined)
        drawAxis(p, vw, v0, 'W', wColor);
}
function generateAxisModel(x, y, z, w) {
    if (w == undefined) {
...
function projectToScreen(model, screen, camera3D, camera4D) {
    let clipPositions;
    if (camera4D) {
        const clip3DPosition = camera4D.objectToClip(model);
        clipPositions = camera3D.objectToClip(clip3DPosition);
    }
    else {
        clipPositions = camera3D.objectToClip(model);
    }
...
function generatePolys(points, faces) {
    const polys = [];
    faces.forEach(f => {
...
function render(model, screen, camera3D, camera4D, brush) {
    if (brush) {
        const screenPositions = projectToScreen(model, screen, camera3D, camera4D);
        const polys = generatePolys(screenPositions, model.mesh.faces);
        drawPoly(screen, polys, brush);
    }
    else {
        const screenPositions = projectToScreen(model, screen, camera3D);
        const polys = generatePolys(screenPositions, model.mesh.faces);
        const _brush = camera4D;
        drawPoly(screen, polys, _brush);
    }
}
function renderFrame(model, screen, camera3D, camera4D) {
    let screenPositions;
    if (camera4D) {
        screenPositions = projectToScreen(model, screen, camera3D, camera4D);
    }
    else {
        screenPositions = projectToScreen(model, screen, camera3D);
    }
    renderVertice(screen, screenPositions);
    renderEdges(screen, screenPositions, model.mesh.edges);
}
function renderFaces(p, screenPositions, faces, brush) {
    const polys = generatePolys(screenPositions, faces);
    drawPoly(p, polys, brush);
}
function renderVertice(p, screenPositions, brush) {
    p.push();
...
function renderEdges(p, screenPositions, edges, brush) {
    p.push();
    if (brush && brush.isStroke) {
        p.stroke(brush.stroke);
        p.strokeWeight(brush.strokeWeight);
...
function convex_hull(points) {
    ...
    while (1) {
        ...
        for (var i = 0; i < points.length; i++) {
            ...
            var d = vec_cross([cur[0], cur[1], 0], [bes[0], bes[1], 0]);
            if (d[2] > 0)
                c = i;
        }
        end = c;
...
function drawPolyWithClip(p, camera, _p, _camera, cube) {
    const polys = [];
    ...
    function projectPolyToScreen(polys, VPMatrix, p) {
        polys.forEach(poly => {
...
function drawPoly(p, polys, brush) {
    p.push();
    p.blendMode(p.BLEND);
    if (brush) {
        if (brush.isStroke) {
