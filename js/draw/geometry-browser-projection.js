import { BufferGeometry, Color, Group, Mesh, PerspectiveCamera, Scene, WebGLRenderer, BufferAttribute, RawShaderMaterial, Vector3, } from 'three';
...
class TesseractGeometry extends BufferGeometry {
    ...
    /**
        ...
        */
    constructor(length = 1, width = 1, height = 1, stack = 1) {
        super();
        this.length = length;
        this.width = width;
        this.height = height;
...
const guis = {
    Tesseract: function (transform4D, mesh) {
        const data = {
...
function chooseFromHash(transform4D, mesh) {
...
function setCameraUniforms(material, transform4D, camera4D, camera) {
...
function initMaterialGUI(gui, material) {
    ...
    for (let i = 0; i < material.uniforms.pointLights.value.length; i++) {
        const pointLight = material.uniforms.pointLights.value[i];
...
function initCameraGUI(gui, camera4D) {
