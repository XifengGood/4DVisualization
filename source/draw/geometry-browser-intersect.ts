...
const guis: Map<(model4D: Model4D, mesh: Group<Object3DEventMap>) => void> = {
    Tesseract: function (model4D: Model4D, mesh: Group) {
        const data = {
    ...
    HyperSphere: function (model4D: Model4D, mesh: Group) {
        const data = {
...
function chooseFromHash(model4D: Model4D, mesh: Group) {
...
function getIntersectGeometry(model4D: Model4D) {
...
function updateIntersectGeometry(model4D: Model4D, mesh: Group) {
    const geometry = getIntersectGeometry(model4D);

    (<Mesh>mesh.children[0]).geometry.dispose();
    (<Mesh>mesh.children[1]).geometry.dispose();
    (<Mesh>mesh.children[0]).geometry = new WireframeGeometry(geometry);
    (<Mesh>mesh.children[1]).geometry = geometry;
}
