...
import { Shader, vsSource, fsSource, UniformData, UniformDataType, vsSource3d, fsSource3d } from "./shader/shader.js";
import { Model3D, Model4D } from "./component/model.js";
import { MeshRenderer3D, MeshRenderer4D, RenderObject } from "./component/meshRender.js";
...
function main() {
    ...
    const canvas = <HTMLCanvasElement>document.querySelector("#tesseract");
...
function main_tessract_unfold() {
    const canvas = <HTMLCanvasElement>document.querySelector("#tesseract-unfold");
    const gl = canvas.getContext("webgl2")!;
    ...
    function draw_opaque_objects_3D(gl: WebGL2RenderingContext, objects: Array<RenderObject>, camera3D: Camera3D) {
        ...
        objects.forEach(object => {
            ...
            drawGPUInstance(transforms, VPMatrix, <MeshRenderer3D>object.meshRenderer);
        });
    }
    function drawGPUInstance(transforms: Transform3D[], VPMatrix: mat4, meshRenderer: MeshRenderer3D) {
    ...
    function setUniforms(shader: Shader, MVPMatrix: mat4) {

        const uniformsData = Array<UniformData>();
        uniformsData.push({
            name: "uMVPMatrix",
            value: MVPMatrix,
...
function main_tessract_eight_cells() {
    ...
    const canvas = <HTMLCanvasElement>document.querySelector("#tesseract-eight-cells");
    const gl = <WebGL2RenderingContext>canvas.getContext("webgl2");
    const rotator = new SimpleRotator(canvas, () => { }, 2);
    const switches = <Element>document.querySelector("#tesseract-eight-cell-switches");
    ...
    const unselectColor: [number, number, number, number] = [1, 1, 1, 0.05];
    const selectColor: [number, number, number, number] = [0.1, 1, 0.1, 0.7];
...
function main_tessract_rotate_a_round() {
    ...
    const canvas = <HTMLCanvasElement>document.querySelector("#tesseract-rotate-a-round");
    const gl = <WebGL2RenderingContext>canvas.getContext("webgl2");
    const slider = <HTMLInputElement>document.querySelector('#yw-angle-slider');
...
function main_tessract_intersection() {
    const canvas = <HTMLCanvasElement>document.querySelector("#tesseract-intersection");
    const gl = <WebGL2RenderingContext>canvas.getContext("webgl2");
    const wSlider = <HTMLInputElement>document.querySelector('#tesseract-intersection-w-slider');
    const xwSlider = <HTMLInputElement>document.querySelector('#tesseract-intersection-xw-slider');
    const ywSlider = <HTMLInputElement>document.querySelector('#tesseract-intersection-yw-slider');
    const zwSlider = <HTMLInputElement>document.querySelector('#tesseract-intersection-zw-slider');
    const rotator = new SimpleRotator(canvas, () => { }, 4);
    const camera3D = new Camera3D(
        {
    ...
    function draw_opaque_objects_3D(gl: WebGL2RenderingContext, objects: Array<RenderObject>, camera3D: Camera3D) {
        ...
        objects.forEach(object => {
            const shader = object.meshRenderer.shader;
            const transform: Transform3D = <Transform3D>object.meshRenderer.model.transform;
            const ModelMatrix = transform.getModelMatrix();
            const MVPMatrix = mat4.create();
            mat4.multiply(MVPMatrix, VPMatrix, ModelMatrix);
    ...
    function draw_transparent_objects_3D(gl: WebGL2RenderingContext, objects: Array<RenderObject>, camera3D: Camera3D) {
        gl.enable(gl.CULL_FACE);
        gl.cullFace(gl.BACK);
        gl.enable(gl.DEPTH_TEST);
        gl.depthMask(false);
...
function intersect(worldPos: Array<vec4>, edges: Array<[number, number]>) {
...
function updateIntersectMesh(model4D: Model4D, mesh3D_edge: Mesh3D, mesh3D_triangle: Mesh3D) {
    const worldPositions = model4D.transform.transform(model4D.mesh.vertice);
    const _edges: Array<[number, number]> = [];
    model4D.mesh.edges.forEach(e => {
        _edges.push([e.indice[0], e.indice[1]]);
    })
    const convexHull: ConvexHull = intersect(worldPositions, _edges);

    const faceColor: [number, number, number, number] = [1, 1, 1, 0.5];
    const edgeColor: [number, number, number, number] = [0, 0, 0, 1];

    const verticeEdge: Array<Vertex3D> = [];
    const verticeTriangle: Array<Vertex3D> = [];
    const edges: Array<Edge> = []
    const triangles: Array<Face> = [];
...
function draw_opaque_objects(gl: WebGL2RenderingContext, objects: Array<RenderObject>, camera4D: Camera4D, camera3D: Camera3D) {
    gl.enable(gl.CULL_FACE);
    gl.cullFace(gl.BACK);
    gl.enable(gl.DEPTH_TEST);
    gl.depthMask(true);
...
function draw_transparent_objects(gl: WebGL2RenderingContext, objects: Array<RenderObject>, camera4D: Camera4D, camera3D: Camera3D) {
    gl.enable(gl.CULL_FACE);
    gl.cullFace(gl.BACK);
    gl.enable(gl.DEPTH_TEST);
    gl.depthMask(false);
...
function setCameraUniforms(shader: Shader, camera4D: Camera4D, camera3D: Camera3D) {
    const uniformsData = Array<UniformData>();
    uniformsData.push({
