...
function isVec3(arg: vec3 | vec3[] | vec4 | vec4[]): arg is vec3 {
    return Array.isArray(arg) && arg.length === 3 && typeof arg[0] === 'number' && typeof arg[1] === 'number' && typeof arg[2] === 'number';
}
function isVec4(arg: vec3 | vec3[] | vec4 | vec4[]): arg is vec4 {
...
class Camera3D {
    ...
    constructor({
        ...
        perspectiveParam = { fovy: (90 * Math.PI) / 180, aspect: 1 },
        near = 0.1,
        far = 100,
    ...
    worldToClip(worldPos: vec3): vec4;
    worldToClip(worldPos: vec4): vec4;
    worldToClip(worldPos: vec3[]): vec4[];
    worldToClip(worldPos: vec4[]): vec4[];
    worldToClip(worldPos: vec3 | vec3[] | vec4 | vec4[]) {
        let _isVec3 = false;
        if (_isVec3 = isVec3(worldPos) || isVec4(worldPos)) {
            let _worldPos = worldPos;
            if (_isVec3)
                _worldPos = toHomogeneous(<vec3>_worldPos);

            const VPMatrix = this.getVPMatrix();
            const clipPos = vec4.create();
            vec4.transformMat4(clipPos, <vec4>_worldPos, VPMatrix);
            ...
            const clipPositions = Array<vec4>();
            const worldPositions = Array<vec4>();
            if (_isVec3) {
                worldPos.forEach(p => {
                    worldPositions.push(toHomogeneous(<vec3>p));
                })
            }else{
                worldPos.forEach(p => {
                    worldPositions.push(<vec4>p);
                })
            }
    ...
    worldToScreen(worldPos: vec3, screen: Screen) {
    ...
    objectToClip(model: Model3D): vec4[];
    objectToClip(clip3DPostions: vec4[]): vec4[];
    objectToClip(args: Model3D | vec4[]) {
    ...
    getProjectionMatrix() {
        switch (this.type) {
            ...
            default: return assertNever(<never>this.type);
        }
    }
...
interface ProjectionMatrix4DInfo {
    projectionMatrix4D: mat4,
    projectParam: {
        translate: vec4,
        m5x1_5x4: vec4,
        m5x5: number
...
class Camera4D {
    ...
    constructor({
        ...
        type = ProjectionType.Orthographic,
        frustum = { left: -1, right: 1, bottom: -1, top: 1, back: -1, front: 1 },
        perspectiveParam = { xField: 90, yField: 90, zField: 90, },
        inside = 0.1,
        outside = 100,
        lookAtPoint = vec4.create()
    } = {}
    ) {
        this.transform = transform;
        this.type = type;
    ...
    objectToClip(model: Model4D): vec4[];
    objectToClip(transform: Transform4D, objectPos: vec4): vec4;
    objectToClip(args: Transform4D | Model4D, objectPos?: vec4): vec4 | vec4[] {
    ...
    getViewMatrix4DWithoutTranslate() {
        ...
        const viewMatrix4D = mat4.fromValues(
            right[0], up[0], forward[0], outside[0],
            right[1], up[1], forward[1], outside[1],
            right[2], up[2], forward[2], outside[2],
            right[3], up[3], forward[3], outside[3],
        )
        return viewMatrix4D;
    }
    ...
    getOrthographicMatrix4D() {
        ...
        const m5x5 = 1;
        const projectionInfo: ProjectionMatrix4DInfo = {
    ...
    getFrustumMatrix4D() {
        ...
        const projectionMatrix = mat4.fromValues(
            ...
            0, 2 * inside * tb, 0, 0,
            0, 0, 2 * inside * fb, 0,
            -(left + right) * rl, -(bottom + top) * tb, -(back + front) * fb, (outside + inside) * fn
        );
        const translate = vec4.fromValues(x, y, z, w);
