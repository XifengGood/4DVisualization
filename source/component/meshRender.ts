import { Shader, UniformData, UniformDataType } from "../shader/shader";
import { Model3D, Model4D } from "./model";
import { Transform3D, Transform4D } from "./transform";
export { MeshRenderer4D, MeshRenderer3D, MeshRenderer }
export interface RenderObject { name: string; meshRenderer: MeshRenderer };
...
interface MeshRenderer {
    model: { transform: Transform4D | Transform3D }
    shader: Shader;
    drawEdge(): void;
    drawTriangle(): void;
}
class MeshRenderer4D implements MeshRenderer {
    attributionBuffer: AttributionBuffer | undefined;
    edgeIndiceBuffer: IndexBuffer | undefined;
    triangleIndiceBuffer: IndexBuffer | undefined;
    constructor(
        public model: Model4D,
    ...
    initAttributeBuffers() {
        ...
        const attributionBuffer: AttributionBuffer = {
    ...
    initEdgeIndiceBuffer() {
        ...
        const edgeIndiceBuffer: IndexBuffer = {
    ...
    initTriangleIndiceBuffer() {
        ...
        const triangleIndiceBuffer: IndexBuffer = {
            indexBuffer: indexBuffer!,
            count: indice.length,
        }
    ...
    drawEdge() {
        if (!this.attributionBuffer) this.initAttributeBuffers();
        ...
        if (!this.edgeIndiceBuffer) this.initEdgeIndiceBuffer();;
    ...
    drawTriangle() {
        ...
        if (!this.triangleIndiceBuffer) this.initTriangleIndiceBuffer();;
    ...
    private setAttributions(attributeBuffer: AttributionBuffer) {
        const gl = this.shader.gl;
...
class MeshRenderer3D implements MeshRenderer {
    attributionBuffer: AttributionBuffer | undefined;
    edgeIndiceBuffer: IndexBuffer | undefined;
    triangleIndiceBuffer: IndexBuffer | undefined;
    constructor(
    ...
    updateAttributeBuffers() {
        if (!this.attributionBuffer) return this.initAttributeBuffers();
        const gl = this.shader.gl;
        const positions = Array<number>();
        const colors = Array<number>();
        const normals = Array<number>();
    ...
    updateEdgeIndiceBuffer() {
        if (!this.edgeIndiceBuffer) return this.initEdgeIndiceBuffer();;
        const gl = this.shader.gl;
        const indice = Array<number>();
        this.model.mesh.edges.forEach(e => {
            indice.push(e.indice[0]);
    ...
    updateTriangleIndiceBuffer() {
        if (!this.triangleIndiceBuffer) return this.initTriangleIndiceBuffer();;
        const gl = this.shader.gl;
        const indice = Array<number>();
        this.model.mesh.faces.forEach(f => {
            for (let i = 0; i + 2 < f.indice.length; i++) {
