...
function generateTesseract({
    ...
    const vertice = Array<Vertex4D>();
    for (let i = 0; i < 16; i++) {
        vertice.push(
            new Vertex4D({
                position: vec4.fromValues(
                    ((i & 1) * 2 - 1) / 2 * length,
                    ((i & 2) - 1) / 2 * width,
                    ((i & 4) / 2 - 1) / 2 * height,
                    ((i & 8) / 4 - 1) / 2 * stack,
...
function generateTesseractHardEdge(colors?: Array<[number, number, number, number]>) {
    const _vertice = Array<vec4>();
    for (let i = 0; i < 16; i++) {
        _vertice.push(
            vec4.fromValues(
                ((i & 1) * 2 - 1) / 2,
...
// 生成服从正态分布的随机数
function randomNormalDistribution(mean: number, deviation: number) {
